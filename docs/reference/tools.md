...
| `easyeda_schematic_place_component`     | `core`  | `medium` | Place a library component/device on the active schematic sheet. The bridge auto-assigns the next free designator (library placeholders like "R?" become "R1", "R2", …) — check the returned designator. If annotation ever fails the part keeps its placeholder; fix it with modify_primitive, since the netlist keys nodes by designator and duplicate "R?" merge into one node. |
...
> Place a library component/device on the active schematic sheet. The bridge auto-assigns the next free designator (library placeholders like "R?" become "R1", "R2", …) — check the returned designator. If annotation ever fails the part keeps its placeholder; fix it with modify_primitive, since the netlist keys nodes by designator and duplicate "R?" merge into one node.
