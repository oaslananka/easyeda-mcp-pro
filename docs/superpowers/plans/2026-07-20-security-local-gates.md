...
- [ ] Run `semgrep --validate --config .semgrep.yml`, `semgrep --test --config .semgrep.yml tests/semgrep`, and a full scan; verify all complete successfully with zero production findings.
...
- [ ] Add `security:semgrep`, `security:semgrep:test`, `security:snyk:oss`, `security:snyk:code`, and `security:snyk` scripts with Snyk `1.1306.1` pinned.
