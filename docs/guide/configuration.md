...
| `AI_PROVIDER`, `AI_MODEL`, `AI_API_KEY`, `AI_ALLOW_DESIGN_MUTATIONS` | Reserved     | No in-process AI provider is called and no AI design mutation is enabled.                       |
| `OTEL_ENABLED`, `OTEL_EXPORTER_OTLP_ENDPOINT`                        | Reserved     | No OTLP exporter is started; local structured metrics remain available.                         |
