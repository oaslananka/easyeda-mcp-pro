...
import { BRIDGE_CONTRACT_VERSION, SUPPORTED_PROTOCOL_VERSIONS, type BridgeHello } from './protocol.js';
...
export class CdpBridgeManager extends EventEmitter {
  ...
  async connect(): Promise<void> {
    ...
    if (!target?.webSocketDebuggerUrl) {
      ...
      throw new Error('CDP bridge could not find an EasyEDA editor page target. Start EasyEDA Pro with --remote-debugging-port=9222 and open a project.');
    }
    ...
    logger.info({ title: target.title, url: target.url }, 'cdp bridge connected to EasyEDA renderer');
  ...
  private async dispatchMethod<TParams>(method: string, params: TParams, timeoutMs: number): Promise<unknown> {
    ...
    if (WRITE_METHOD_RE.test(method) && process.env.EASYEDA_CDP_ALLOW_UNMAPPED_WRITES !== 'true') {
      throw new Error(`CDP bridge has not mapped mutating EasyEDA method "${method}" yet. Use api.execute for explicit runtime probes, or set EASYEDA_CDP_ALLOW_UNMAPPED_WRITES=true only in a disposable test project.`);
    }
  ...
  private selectEasyedaTarget(targets: CdpTarget[]): CdpTarget | undefined {
    ...
    return (
      targets.find((target) => target.type === 'page' && target.url.includes(EASYEDA_EDITOR_URL_PART)) ??
  ...
  private cdp(method: string, params: Record<string, unknown> = {}, timeoutMs = this.config.BRIDGE_TIMEOUT_MS): Promise<CdpResponse> {
  ...
  private async evaluateObject<T = unknown>(expression: string, timeoutMs = this.config.BRIDGE_TIMEOUT_MS): Promise<T> {
    ...
    return (result?.result?.value ?? result?.result?.unserializableValue ?? result?.result?.description) as T;
  }
  ...
  private requireMappedWriteAllowed(method: string): void {
    if (process.env.EASYEDA_CDP_ALLOW_WRITES !== 'true') {
      throw new Error(`CDP mapped write method "${method}" requires EASYEDA_CDP_ALLOW_WRITES=true. Use only on a disposable EasyEDA test project.`);
    }
  }
